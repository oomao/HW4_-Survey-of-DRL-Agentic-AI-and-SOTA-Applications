...
- `code/test_harness.py` — 20 個 pytest（工具單元 + schema-validated dispatch 守衛 + pipeline 不變量：無重複、引用都真實、critic 確實觸發、offline 決定性）
- `artifacts/demo_run.md` — 一次完整的 demo trace（query：「Survey robotics VLA models 2024-2026」）

**MVP scope**（刻意保持小）：

- ~560 行的 4-phase orchestrator（`harness_demo.py`）+ ~650 行工具層（`tools.py`：15 篇 corpus、JSON schema、validated dispatch）；
